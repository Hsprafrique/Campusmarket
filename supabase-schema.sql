...
CREATE TABLE IF NOT EXISTS profiles (
  ...
  store_activated_at TIMESTAMPTZ,
  store_payment_ref TEXT,
  campus TEXT,
...
-- =============================================
-- SITE VISITS (for admin visitor analytics)
-- =============================================
...
CREATE POLICY "Anyone can record a visit" ON site_visits
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Admins can view all visits" ON site_visits
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  );
