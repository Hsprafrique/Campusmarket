...
export default function MyStore() {
  ...
  const { data: store, isLoading: storeLoading } = useQuery({
    ...
    onSuccess: (data) => {
      if (data) setForm({
        ...
        campus: data.campus || '', whatsapp_number: data.whatsapp_number || '',
  ...
  // Pre-fill from user profile if no store yet
  useEffect(() => {
    if (!storeLoading && !store && user) {
      setForm(prev => ({
        ...prev,
  ...
  }, [storeLoading, store, user]);

  const { data: storeListings = [] } = useQuery({
    queryKey: ['storeListings', store?.id],
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['myStore', rawUser?.id]);
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-600">{store ? 'Manage your store profile and listings' : 'Set up your campus store to build your brand'}</p>
            </div>
          </div>
          ...
          {store && (
            <div className="relative mb-8 rounded-2xl overflow-hidden shadow-lg">
              {store.banner_url ? (
                <img src={store.banner_url} alt="Store banner" className="w-full h-40 object-cover" />
              ) : (
                <div className="w-full h-40 bg-gradient-to-r from-purple-600 to-orange-500" />
              )}
              <div className="absolute bottom-4 left-6 flex items-end gap-4">
                {store.logo_url ? (
                  <img src={store.logo_url} alt="Store logo" className="w-20 h-20 rounded-xl object-cover border-4 border-white shadow-md" />
                ) : (
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>{store ? 'Update your store details' : 'Fill in your store information'}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-5">
                  <div className="space-y-2">
                    <Label>Store Name *</Label>
                    <Input placeholder="e.g., Tunde's Tech Store" value={form.name}
                      onChange={e => setForm({ ...form, name: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                    <Label>Description</Label>
                    <Textarea placeholder="Tell buyers what you sell..." value={form.description}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input placeholder="2348012345678" value={form.whatsapp_number}
                      onChange={e => setForm({ ...form, whatsapp_number: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                    <Label className="flex items-center gap-1"><Instagram className="w-4 h-4" />Instagram Handle</Label>
                    <Input placeholder="@yourstore" value={form.instagram_handle}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <label className="flex-1 border-2 border-dashed border-gray-300 rounded-xl p-4 flex flex-col items-center cursor-pointer hover:border-purple-400 hover:bg-purple-50 transition-colors">
                        {uploadingLogo ? <Loader2 className="w-6 h-6 animate-spin text-gray-400" /> : (
                          <><ImagePlus className="w-6 h-6 text-gray-400 mb-1" /><span className="text-xs text-gray-500">Upload logo</span></>
                        )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <label className="flex-1 border-2 border-dashed border-gray-300 rounded-xl p-4 flex flex-col items-center cursor-pointer hover:border-purple-400 hover:bg-purple-50 transition-colors">
                        {uploadingBanner ? <Loader2 className="w-6 h-6 animate-spin text-gray-400" /> : (
                          <><ImagePlus className="w-6 h-6 text-gray-400 mb-1" /><span className="text-xs text-gray-500">Upload banner (wide image)</span></>
                        )}
                  ...
                  <Button className="w-full bg-purple-600 hover:bg-purple-700 h-12"
                    ...
                    {saveMutation.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                      <><Save className="w-4 h-4 mr-2" />{store ? 'Update Store' : 'Create Store'}</>
                    )}
                  </Button>

                  {store && (
                    <Button variant="outline" className="w-full text-red-600 hover:bg-red-50 border-red-200"
                      onClick={() => { if (window.confirm('Delete your store? This cannot be undone.')) deleteMutation.mutate(); }}
